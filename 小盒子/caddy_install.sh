...
Download_caddy() {
  ...
  [[ -e "caddy*" ]] && rm -rf "caddy*"
  if [[ ${bit} == "x86_64" ]]; then
    wget --no-check-certificate -O "caddy" "https://github.com/Spritesmine/Caddy_Linux/releases/latest/download/caddy_v2_linux_amd64"
  elif [[ ${bit} == "i386" || ${bit} == "i686" ]]; then
    wget --no-check-certificate -O "caddy" "https://github.com/Spritesmine/Caddy_Linux/releases/latest/download/caddy_v2_linux_386"
  elif [[ ${bit} == "armv7l" ]]; then
    wget --no-check-certificate -O "caddy" "https://github.com/Spritesmine/Caddy_Linux/releases/latest/download/caddy_v2_linux_armv7"
  elif [[ ${bit} == "arm64" || ${bit} == "aarch64" ]]; then
    wget --no-check-certificate -O "caddy" "https://github.com/Spritesmine/Caddy_Linux/releases/latest/download/caddy_v2_linux_arm64"
  else
...
Service_caddy() {
  if [[ ${release} == "centos" ]]; then
    if ! wget --no-check-certificate https://raw.githubusercontent.com/Spritesmine/ServerStatus-Jasmine/master/caddy/caddy_centos -O /etc/init.d/caddy; then
    ...
    if ! wget --no-check-certificate https://raw.githubusercontent.com/Spritesmine/ServerStatus-Jasmine/master/caddy/caddy_debian -O /etc/init.d/caddy; then
      echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy服务 管理脚本下载失败 !" && exit 1
    fi
    chmod +x /etc/init.d/caddy
