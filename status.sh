...
#  Version: Test v0.4.1
#  Author: Toyo, Modified by QISANSUI
#=================================================
...
github_prefix="https://raw.githubusercontent.com/Spritesmine/ServerStatus-Jasmine/master"
coding_prefix="https://cokemine.coding.net/p/agony/d/ServerStatus-Jasmine/git/raw/master"
...
Download_Server_Status_server() {
  cd "/tmp" || exit 1
  [[ ${mirror_num} == 2 ]] && bundle_link="https://qisansui.coding.net/p/agony/d/ServerStatus-Jasmine/git/archive/master/?download=true" || bundle_link="https://github.com/Spritesmine/ServerStatus-Jasmine/archive/master.zip"
  ...
  unzip master.zip
  rm -rf master.zip
  [[ -d "/tmp/qisansui-agony-ServerStatus-Jasmine-master" ]] && mv "/tmp/qisansui-agony-ServerStatus-Jasmine-master" "/tmp/ServerStatus-Jasmine-master"
  [[ ! -d "/tmp/ServerStatus-Jasmine-master" ]] && echo -e "${Error} ServerStatus 服务端解压失败 !" && exit 1
  cd "/tmp/ServerStatus-Jasmine-master/server" || exit 1
  make
  [[ ! -e "sergate" ]] && echo -e "${Error} ServerStatus 服务端编译失败 !" && cd "${file_1}" && rm -rf "/tmp/ServerStatus-Jasmine-master" && exit 1
  cd "${file_1}" || exit 1
  mkdir -p "${server_file}"
  if [[ -e "${server_file}/sergate" ]]; then
    mv "${server_file}/sergate" "${server_file}/sergate1"
    mv "/tmp/ServerStatus-Jasmine-master/server/sergate" "${server_file}/sergate"
  else
    mv "/tmp/ServerStatus-Jasmine-master/server/sergate" "${server_file}/sergate"
    wget -N --no-check-certificate "${github_link}/Spritesmine/Jasmine-Theme/releases/latest/download/Jasmine-Theme.zip"
    unzip Jasmine-Theme.zip && mv "./Jasmine-Theme" "${web_file}"
    rm -rf Jasmine-Theme.zip
  fi
  rm -rf "/tmp/ServerStatus-Jasmine-master"
  if [[ ! -e "${server_file}/sergate" ]]; then
